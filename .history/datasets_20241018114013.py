...
def load_USPS_data(path, args):
    mat = sio.loadmat(path)
    data_list = [mat['X1'].astype('float32'), mat['X2'].astype('float32')]
    data_list = [normalize(dv).astype('float32') for dv in data_list]
    labels = np.squeeze(mat['Y']).reshape(5000,)
    dims = [dv.shape[1] for dv in data_list]
    data_size = labels.shape[0]
    class_num = len(np.unique(labels))
    if np.max(labels) == class_num:
...
def load_Caltech101_data(path, args):
    mat = sio.loadmat(path)
    data_list = [mat['X1'].astype('float32'), mat['X2'].astype('float32')]
    data_list = [normalize(dv).astype('float32') for dv in data_list]
    labels = np.squeeze(mat['Y']).astype('int')
    dims = [dv.shape[1] for dv in data_list]
    data_size = labels.shape[0]
    class_num = len(np.unique(labels))
    if np.max(labels) == class_num:
...
def load_Fashion_data(path, args):
    mat = sio.loadmat(path)
    data_list = [mat['X1'].astype('float32').reshape(10000,784), mat['X2'].astype('float32').reshape(10000,784), mat['X3'].astype('float32').reshape(10000,784)]
    # data_list = [normalize(dv).astype('float32') for dv in data_list]
    labels = np.squeeze(mat['Y']).astype('int').reshape(10000, )
    dims = [dv.shape[1] for dv in data_list]
    data_size = labels.shape[0]
    class_num = len(np.unique(labels))
    if np.max(labels) == class_num:
...
def load_NoisyMNIST_data(path, args):
    mat = sio.loadmat(path)
    data_list = [mat['X1'].astype('float32').reshape(50000,784), mat['X2'].astype('float32').reshape(50000,784)]
    # data_list = [normalize(dv).astype('float32') for dv in data_list]
    labels = np.squeeze(mat['trainLabel']).astype('int').reshape(50000, )
    dims = [dv.shape[1] for dv in data_list]
    data_size = labels.shape[0]
    class_num = len(np.unique(labels))
    if np.max(labels) == class_num:
...
def build_dataset(args):
    ...
    if args.dataset_name == 'Caltech7-5V':
        ...
        data_list, labels = load_Reuters_data('/home/yanwenbiao/CODE/2021-CVPR-MvCLN/datasets/Reuters_dim10.mat', args)
        data_list = [StandardScaler().fit_transform(dv) for dv in data_list]
    elif args.dataset_name == 'BDGP':
        data_list, labels = load_BDGP_data('/data1/yanwenbiao/CODE/2020-NIPS-PVC/data/BDGP.mat', args)
        # data_list = [StandardScaler().fit_transform(dv) for dv in data_list]
    elif args.dataset_name == 'RGBD':
        data_list, labels = load_RGBD_data('/home/yanwenbiao/CODE/IncompleteMVC/data/RGB-D.mat', args)
        data_list = [StandardScaler().fit_transform(dv) for dv in data_list]
    elif args.dataset_name == 'MNIST-USPS':
        data_list, labels = load_USPS_data('/data1/yanwenbiao/CODE/2020-NIPS-PVC/data/MNIST-USPS.mat', args)
        # data_list = [StandardScaler().fit_transform(dv) for dv in data_list]
    elif args.dataset_name == 'Caltech101-20':
        data_list, labels = load_Caltech101_data('/data1/yanwenbiao/CODE/2020-NIPS-PVC/data/Caltech101-20.mat', args)
        # data_list = [StandardScaler().fit_transform(dv) for dv in data_list]
    elif args.dataset_name == 'LandUse_21':
        data_list, labels = load_LandUse_21_data('/home/yanwenbiao/CODE/2021-CVPR-Completer/data/LandUse-21.mat', args)
        data_list = [StandardScaler().fit_transform(dv) for dv in data_list]
    elif args.dataset_name == 'Fashion':
        data_list, labels = load_Fashion_data('data/Fashion.mat', args)
        data_list = [StandardScaler().fit_transform(dv) for dv in data_list]
    elif args.dataset_name == 'NoisyMNIST':
        data_list, labels = load_NoisyMNIST_data('data/NoisyMNIST.mat', args)
        data_list = [StandardScaler().fit_transform(dv) for dv in data_list]
    elif args.dataset_name == 'Caltech_2V':
        data_list, labels = load_Caltech2v_data('/home/yanwenbiao/CODE/MFLVC-main/data/Caltech-5V.mat', args)
        data_list = [StandardScaler().fit_transform(dv) for dv in data_list]
