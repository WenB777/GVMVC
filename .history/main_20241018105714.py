...
def initialization(model, mv_loaders, cmv_data, args):
    ...
    with torch.no_grad():
        ...
        # labels = kmeans.fit_predict(fused_latent_representations)
        # cluster_centers = kmeans.cluster_centers_

        # # 初始化 mu
        # model.mu.data = torch.tensor(cluster_centers, dtype=torch.float32).to(args.device)

        # # 初始化 pi
        # counts = torch.tensor([(labels == i).sum() for i in range(args.class_num)], dtype=torch.float32).to(args.device)
        # model.pi.data = counts / counts.sum()

        # # 初始化 logvar
        # logvar_list = []
        # for i in range(args.class_num):
        #     cluster_data = fused_latent_representations[labels == i]
        #     if len(cluster_data) > 1:  # 簇内有多个点时计算方差
        #         cluster_var = cluster_data.var(axis=0)
        #     else:  # 簇内只有一个点时用全局方差代替
        #         cluster_var = fused_latent_representations.var(axis=0)
        #     logvar_list.append(cluster_var)

        # # 转换为 NumPy 数组然后转换为 PyTorch 张量
        # logvar_array = np.array(logvar_list)
        # logvar_tensor = torch.tensor(logvar_array, dtype=torch.float32).to(args.device)
        # model.logvar.data = torch.log(logvar_tensor)
